...
Behavioral Summary 1. Sequential Thinking MCP — for structured logic and step planning. 2. Context7 MCP — for always-up-to-date documentation and version correctness. 3. Serena MCP — for local environment management, scaffolding, dependency control, and file operations. 4. mcp-feedback-enhanced — for iterative improvement and user alignment.
...
• Prefer Adaptive Platform UI widgets: - `moneko-mobile/docs/adaptive_platform_ui_widgets_example.md` - `moneko-mobile/docs/adaptive_platform_ui_widgets.md`
...
Async UI And Loading Rules (Strict)

    •	Read `../docs/async-ui-and-optimistic-mutation-contract.md` before changing any async UI, provider refresh, local cache, repository mutation, or outbox flow.
    •	When no trustworthy cached value exists, use a layout-matched skeleton that reserves the final content geometry.
    •	Never present temporary zeroes, false flags, empty collections, placeholder labels, or default amounts as authoritative data while a read is unresolved.
    •	When cached or previously loaded data exists, retain it during refresh and use stale-while-revalidate. Do not replace it with a skeleton, spinner-only page, or blank state.
    •	Reserve stable space for unknown badges, counts, totals, cards, and list rows so loading resolution cannot cause a visible jump or false-value flip.
    •	Use a progress indicator only for an indeterminate action without a predictable content layout. Data-shaped loading surfaces require skeletons.

Optimistic Mutation Rules (Strict)

    •	Every deterministic app-data mutation must update SQLite, provider state, and visible derived state immediately before backend dispatch.
    •	Persist an idempotent outbox operation whenever the mutation supports offline retry.
    •	Retryable and offline failures remain optimistic, visibly pending, and queued for reconciliation.
    •	A non-retryable backend rejection must restore the exact pre-mutation snapshot and emit an app-level error toast even if the originating route or sheet has closed.
    •	Track a mutation ID and per-entity revision. Roll back only when the rejected mutation still owns the current revision; stale failures must trigger reconciliation rather than overwrite newer state.
    •	Refresh signals and derived caches must preserve optimistic overlays until the backend confirms or terminally rejects them.
    •	Authentication, subscription purchase, bank transfer, and externally authoritative operations must enter a visible pending state immediately but must not claim success before external confirmation.
    •	Tests are mandatory for immediate projection, success reconciliation, retryable queuing, terminal rollback, and overlapping mutations.
