...
void main() {
  ...
  group('InitCacheManager', () {
    ...
    test('loadStaleBestEffort loads expired-but-recent cache', () async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
      final manager = InitCacheManager(prefs);
      ...
      expect(manager.load('1.0.0+1'), isNull);
      expect(manager.loadBestEffort('1.0.0+1'), isNull);
      expect(manager.loadStaleBestEffort('2.0.0+1'), isNotNull);
    });

    test('loadStaleBestEffort rejects very old cache', () async {
      final now = DateTime.now().millisecondsSinceEpoch;
      SharedPreferences.setMockInitialValues({
        'app_init_cache_v2':
            jsonEncode({'timestamp': DateTime.now().toIso8601String()}),
        'app_init_cache_timestamp_v2': DateTime.now()
      ...
      expect(manager.loadStaleBestEffort('2.0.0+1'), isNull);
      expect(
          now, isNonZero); // keep analyzer happy about unused var in this test
    });
  });
}
