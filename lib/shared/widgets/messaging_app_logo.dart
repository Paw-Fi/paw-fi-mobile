...
enum MessagingAppLogoType { telegram, whatsapp }

class MessagingAppLogo extends StatelessWidget {
  const MessagingAppLogo({
    super.key,
    required this.type,
    required this.color,
    this.size = 20,
  ...
  final MessagingAppLogoType type;
  final Color color;
  final double size;
  ...
  Widget build(BuildContext context) {
    return SvgPicture.string(
      switch (type) {
        MessagingAppLogoType.telegram => _telegramSvg,
        MessagingAppLogoType.whatsapp => _whatsappSvg,
      },
      width: size,
      height: size,
