...
class TransactionsPieChart extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<TransactionsPieChart> createState() =>
      _TransactionsPieChartState();
}
...
class _TransactionsPieChartState extends ConsumerState<TransactionsPieChart> {
  int? _touchedIndex;

  void _showBreakdownSheet(BuildContext context, double totalSpent) {
    if (widget.currencyTypeTotals.length <= 1) return;
    ...
    showMultiCurrencyTotalBreakdownSheet(
      ...
      currencyTypeTotals: widget.currencyTypeTotals,
      rates: rates,
