  ...
  try {
    ...
    while (true) {
      final response = await client
          .rpc('get_home_mom_transactions_v4', params: <String, dynamic>{
        'p_user_id': query.userId,
        'p_start_date': query.formattedStartDate,
        'p_end_date': query.formattedEndDate,
    ...
    Future<void> fetchFallbackPageSet({List<String>? contacts}) async {
      ...
      while (true) {
        dynamic fallback = client.from('expenses').select(
            ...
            'created_at,updated_at,raw_text,merchant,merchant_id,merchant_structured_name,merchants(domain),split_group_id,parent_recurring_id,'
